...
SHARE.showMsg = () => {
  ...
  SHARE.sysMsgText.textContent = SHARE.msgQueue.shift()
...
SHARE.showTooltip = text => {
  SHARE.tooltip.textContent = text
}
//往左切換頁面
SHARE.toLeft = target => {
  target.className
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  SHARE.changePWBtn = document.getElementById('change-password-btn') // 修改密碼按鈕
  SHARE.logoutBtn = document.getElementById('logout-btn') // 登出按鈕
  SHARE.loggerBtn = document.getElementById('logger-btn') // 操作履歷按鈕
  SHARE.changePWform = document.getElementById('change-password-form') // 修改密碼表單
  SHARE.closePWbtn = document.getElementById('close-pw-btn') // 關閉修改密碼按鈕
  // 元素與事件的綁定
  // 在logo上右鍵連點兩次切換至前臺
  ...
  SHARE.manualBtn.addEventListener('click', () => {
    window.open('/ceb9458d5364668953b05327205aa8af/manual', '_blank')
  })
  //  遊標移過手冊按鈕顯示對應工具提示
  ...
  // 點擊操作履歷按鈕開啓履歷視窗
  SHARE.loggerBtn.addEventListener('click', () => {
    SHARE.logger.classList.add('active')
    setTimeout(() => {
      SHARE.logger.focus()
    }, 100)
  })
  // 遊標移過操作履歷按鈕顯示對應工具提示
  SHARE.loggerBtn.addEventListener('mouseenter', () => {
    SHARE.showTooltip('查看後臺操作記錄')
  })
  // 遊標移出操作履歷按鈕隱藏工具提示
  SHARE.loggerBtn.addEventListener('mouseleave', () => {
    SHARE.hideTooltip()
  })
  // 遊標移過修改密碼按鈕顯示對應工具提示
  SHARE.changePWBtn.addEventListener('mouseenter', () => {
    SHARE.showTooltip('修改密碼')
  })
  // 遊標移出修改密碼按鈕隱藏對應工具提示
  SHARE.changePWBtn.addEventListener('mouseleave', () => {
    SHARE.hideTooltip()
  })
  // 按下修改密碼按鈕彈出修改密碼表單
  SHARE.changePWBtn.addEventListener('click', () => {
    SHARE.changePWform.classList.add('active')
    SHARE.changePWform.children[2].focus()
    setTimeout(() => {
      SHARE.changePWform.classList.remove('underground')
    }, 200)
  })
  // 按下關閉修改密碼按鈕關閉修改密碼表單
  SHARE.closePWbtn.addEventListener('click', () => {
    SHARE.changePWform.classList.remove('active')
    setTimeout(() => {
      SHARE.changePWform.classList.add('underground')
    }, 200)
  })
  // 在修改密碼表單中按下按鍵
  SHARE.changePWform.addEventListener('keydown', e => {
    // 按下ESC關閉表單
    if (e.keyCode === 27) {
      SHARE.changePWform.classList.remove('active')
      setTimeout(() => {
    ...
    // 按下Enter且值不爲空
    } else if (e.keyCode === 13 && SHARE.changePWform.children[2].value !== '' && SHARE.changePWform.children[3].value !== '') {
      // 如果兩次密碼一致
      if (SHARE.changePWform.children[2].value === SHARE.changePWform.children[3].value) {
        let xhr = new XMLHttpRequest()
        xhr.open('POST', '/ceb9458d5364668953b05327205aa8af/change_password', true)
        let fd = new FormData()
        let token = document.getElementsByName("csrf-token")[0].content
        fd.append('authenticity_token', token)
        fd.append('password', SHARE.changePWform.children[2].value)
        fd.append('re_password', SHARE.changePWform.children[3].value)
        SHARE.workQueue++
        SHARE.showLoading()
        xhr.addEventListener('load', e => {
          if (e.target.status === 200) {
            SHARE.workQueue--
            SHARE.showLoading()
            SHARE.pushMsg('密碼修改成功')
            SHARE.changePWform.classList.remove('active')
            setTimeout(() => {
              SHARE.changePWform.classList.add('underground')
              SHARE.changePWform.children[2].value = ''
              SHARE.changePWform.children[3].value = ''
            }, 200)
          } else {
            SHARE.workQueue--
            SHARE.showLoading()
            SHARE.pushMsg('密碼修改失敗')
          }
        })
        xhr.send(fd)
      // 如果兩次密碼不一致
  ...
  // 遊標移過登出按鈕顯示對應工具提示
  SHARE.logoutBtn.addEventListener('mouseenter', () => {
    SHARE.showTooltip('登出')
  })
  // 遊標移出登出按鈕隱藏工具提示
  SHARE.logoutBtn.addEventListener('mouseleave', () => {
    SHARE.hideTooltip()
  })
  // 按下登出按鈕，登出
  SHARE.logoutBtn.addEventListener('click', () => {
  ...
  document.addEventListener('keydown', e => {
    ...
    // 如果按下Shift + 左或右, 移動至左邊或右邊的功能區
    if(e.shiftKey && (e.keyCode === 37 || e.keyCode === 39) && e.target.tagName !== 'INPUT' && e.target.tagName !== 'TEXTAREA') {
      SHARE.pageHandler(e.keyCode)
    // shift+左或右移動至對應功能區
    }else if(e.keyCode === 9 && e.target.tagName !== 'INPUT' && e.target.tagName !== 'TEXTAREA') {
      // 如果在表單中點tab鍵，切換焦點欄位
      e.preventDefault()
      e.stopPropagation()
    }
  ...
  SHARE.logVue = new Vue ({
    ...
    mounted: function() {
      this.getLogs()
      SHARE.logger = document.getElementById('logger-content')
    },
    methods: {
      // 獲取操作履歷
      getLogs: function() {
        SHARE.workQueue++
        SHARE.showLoading()
        this.$http.get('/ceb9458d5364668953b05327205aa8af/get_logs.json').then(response => {
          this.logs = response.body.logs
          SHARE.workQueue--
          SHARE.showLoading()
        }, response => {
          SHARE.pushMsg("發生錯誤無法獲取履歷")
          SHARE.workQueue--
          SHARE.showLoading()
        })
      },
      ...
      closeLogger: function() {
        SHARE.logger.classList.remove('active')
      }
    }
  })
})
